...
public class PhotoUtil {
    ...
    public void takePicture(Activity activity,Context context) {
        try {
            //每次选择图片把之前的图片删除
            clearCropFile(buildUri(activity),context);
            mContext = context;
    ...
    public void selectPicture(Activity activity,Context context) {
        try {
            //每次选择图片把之前的图片删除
            clearCropFile(buildUri(activity),context);
            mContext = context;
   ...
   public boolean isIntentAvailable(Activity activity, Intent intent) {
    ...
    public void onActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            //截图
            case INTENT_CROP:
                if (resultCode == Activity.RESULT_OK && new File(getRealFilePath(mContext,buildUri(activity))).exists()) {
                    this.onPhotoResultListener.onPhotoResult(buildUri(activity));
                }
                break;
        }
    }
     ...
     * @return
     */
    public boolean clearCropFile(Uri uri,Context context) {
        if (uri == null) {
            return false;
        }
